#!/bin/sh
#
# Download and parse Wireshark's MAC vendor database for LAN Scanner.
# This script is intentionally quiet: failures must not interrupt scanning.
...
if command -v uclient-fetch >/dev/null 2>&1; then
	uclient-fetch -T 10 -O "$TMP_GZ" "$URL" >/dev/null 2>&1 && fetch_ok=1
fi
if [ "$fetch_ok" -ne 1 ] && command -v wget >/dev/null 2>&1; then
	wget -T 10 -O "$TMP_GZ" "$URL" >/dev/null 2>&1 && fetch_ok=1
fi
